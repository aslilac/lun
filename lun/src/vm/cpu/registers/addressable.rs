...
pub trait AddressableRegister {
	...
	fn get_reg_and_index(self) -> (VmNativeRegister, u64);
}
...
impl VmByteRegister {
	pub fn new(reg: VmNativeRegister, i: u64) -> Self {
...
impl AddressableRegister for VmByteRegister {
...
impl AddressableRegister for VmQwordRegister {
	const MAX_REGISTER_INDEX: u64 = 4;
...
impl AddressableRegister for VmHwordRegister {
	const MAX_REGISTER_INDEX: u64 = 2;
...
impl AddressableRegister for VmNativeRegister {
...
mod tests {
	use super::*;
	use crate::vm::{Vm, VmNativeRegister::*};
	...
	fn byte_registers() {
		...
		for i in 0..8 {
			assert_eq!(vm.get_register_value(VmByteRegister::new(x, i)), i);
			vm.set_register_value(VmByteRegister::new(y, i), i);
		}

		assert_eq!(vm.get_register_value(y), 0x0706050403020100);

		vm.set_register_value(VmByteRegister::new(y, 4), 0xff);
